  ...
  for (int k = 0; k < sdata->nparticles; k++) {
    for (int l = 0; l < sdata->nparticles; l++) {
      if (l == k)
        continue;
      denominator +=
          1 / (1 + getl2NormDiff(simdata_pos_ptr(sdata, l),
                                 simdata_pos_ptr(sdata, k), sdata->posdim));
    }
  }
...
float *tsne_precompute(simdata_t *sdata, int dataSize) {
  ...
  for (int i = 0; i < dataSize; i++) {
    for (int j = 0; j < dataSize; j++) {
      l2NormDiffs[i * dataSize + j] =
          getl2NormDiff(simdata_feat_ptr(sdata, i), simdata_feat_ptr(sdata, j),
                        sdata->featdim);
    }
  }
...
void getTsneForce(float *force, float *position, int dataIndex,
                  float *positionActor, int actorDataIndex, simdata_t *sdata,
                  float *aux) {
  ...
  float qjiNumerator =
      1 / (1 + getl2NormDiff(position, positionActor, sdata->posdim));
  float qjiDenominator = aux[sdata->nparticles * sdata->nparticles];

  float scalar = 4 *
                 ((pij + pji) / 2 / dataSize - qjiNumerator / qjiDenominator) /
                 (1 + getl2NormDiff(position, positionActor, sdata->posdim));
